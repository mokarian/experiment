...
class Azure:
    ...
    def __init__(self):
        ...
        self.endpoint = 'https://{}.src.windows.net/'.format(self.config["src"]["service_name"])
        self.api_version = '?api-version=2019-05-06'
        self.headers = {'Content-Type': 'application/json',
                        'api-key': '{}'.format(self.config["src"]["api_key"])}
    ...
    def create_index(self, index_name="test"):
        ...
        response = requests.put(url, headers=self.headers,
                                json=self.utils.read_json_from_resources("index-schema.json"))
        return response.json()
    ...
    def make_search(self, misspelled_name, fields, index_name="test"):
        ...
        :query  the query
        """
        url = self.endpoint + "/indexes/" + index_name + "/docs/src" + self.api_version
